...
wdproject    <- paste0(wdmain, "Projects/envquenching/")
...
# Pasta para os dados
pasta = paste0(wdassigndata, tabela, "/zmax_", zmax)
pasta_info = paste0(pasta, "/Info")

# Se não existir, cria
if (!dir.exists(pasta)) {
  dir.create(pasta, recursive = TRUE)  # recursive = TRUE cria pastas intermediárias, se necessário
}

if (!dir.exists(pasta_info)) {
  dir.create(pasta_info, recursive = TRUE)  # recursive = TRUE cria pastas intermediárias, se necessário
}
...
if(tabela == "clean"){
  gals_input_file    <- "clean_SDSS_DR18_Legacy_MGS_QSO_localDensity+GSWLC+Simard11+DS18.csv"
}

if(tabela == "todos"){
  gals_input_file   <- "SDSS_DR18_Legacy_MGS_QSO_localDensity+GSWLC+Simard11+DS18.csv"
}

data_gals_i   <- fread(paste0(wddata, "environmental-quenching-data/", gals_input_file), select = gals_colnames)
...
select_groups_mangle <- mangle_L$groupID_L[which(mangle_L$f_mangle_20rvir_L >= 0.9)]

groupIDs_select_mangle <- groupIDs_select[groupIDs_select %in% select_groups_mangle]
Ngroups_select_mangle  <- length(groupIDs_select_mangle)
print(sprintf('Number of groups (after exluding groups close zmin or zmax): %i', Ngroups_select_mangle))
...
for(group in groupIDs){
  ...
  if(NnearbyGal == 0){
    write(group, file = paste0(pasta_info, "/NoNearbyGal", "_zmax", zmax, "_Rlim", Rlim, "_Ma", Mhalo_min_group, ".csv"), append = T)
  }else{
...
info_groups <- data.frame(info = c("tamanho original", "pós-corte", "longe das bordas em z", "longe das bordas em RA e DEC (mangle)"),
                          quantidade = c(nrow(data_groups_i), nrow(data_groups), Ngroups_select, Ngroups_select_mangle))
...
# salvar só com flag_good == 1
output_single_flag_good <- output_single[which(output_single$flag_good == 1),]
# Número de grupos flag_good == 1
print(sprintf('Number of groups: %i', length(unique(output_single_flag_good$groupID))))

### Definir tabelas de saída ----
output_file           <- paste0(pasta, "/assignment2groups_zmax", zmax, "_Rlim", Rlim, "_Ma", Mhalo_min_group, ".csv")
output_file_flag_good <- paste0(pasta, "/assignment2groups_zmax", zmax, "_Rlim", Rlim, "_Ma", Mhalo_min_group, "_flag_good.csv")
info_groups_output    <- paste0(pasta_info, "/assignment2groups", "_zmax", zmax, "_Rlim", Rlim, "_Ma", Mhalo_min_group, ".infogroups")
info_gals_output      <- paste0(pasta_info, "/assignment2groups", "_zmax", zmax, "_Rlim", Rlim, "_Ma", Mhalo_min_group, ".infogals")
...
write.csv(output_single_flag_good, file = output_file_flag_good, row.names = F)