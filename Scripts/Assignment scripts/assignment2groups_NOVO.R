...
gals_input_file   <- "clean_SDSS_DR18_Legacy_MGS_QSO_kcorr+GSWLC-X2_MGS+Simard11+DS18.csv"
...
output_file_flag_good <- paste0(wdassigndata, "assignment2groups_zmax", zmax, "_Rlim", Rlim, "_Ma", Mhalo_min_group, "_flag_good.csv")
...
groupIDs_select <- groupIDs_select[groupIDs_select %in% select_groups_mangle]
Ngroups_select  <- length(groupIDs_select)
print(sprintf('Number of groups (after exluding groups close zmin or zmax): %i', Ngroups_select))
...
info_groups <- data.frame(info = c("tamanho original", "pós-corte", "longe das bordas e z"),
...
print(sprintf('Number of groups: %i', length(unique(output_single_flag_good$groupID)))) # Número de grupos
