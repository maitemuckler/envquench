...
# Atenção: estou reescrevendo data.s para remover galáxias dessa amostra com sigma > 200 km/s. 
# O modelo já aprendeu com a amostra completa, estou removendo essas somente pq a Marina pediu, por uma questão de bin.

data.s <- subset(data.s, data.s$velDisp_e <= 200) # 23 galáxias removidas
...
# Plotagem
# 1. Adiciona os rótulos dos painéis ao resultados_df
...
# 2. Cria extremos_df com valores min/max
extremos_df <- modelos %>%
  ...
  mutate(painel_logvelDisp_e = as.character(painel_logvelDisp_e)) %>%
...
extremos_df <- extremos_df %>%
  mutate(
    ...
    vjust_label = case_when(
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[1]), ", ", round(10^vetor_prob[2]), ")") & modelo == "fSFG" & tipo_ponto == "min" ~ -0.5,
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[1]), ", ", round(10^vetor_prob[2]), ")") & modelo == "fLTG" & tipo_ponto == "min" ~  1.5,
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[1]), ", ", round(10^vetor_prob[2]), ")") & modelo == "fSFG" & tipo_ponto == "max" ~ -0.5,
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[1]), ", ", round(10^vetor_prob[2]), ")") & modelo == "fLTG" & tipo_ponto == "max" ~  1.6,
      
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[2]), ", ", round(10^vetor_prob[3]), ")") & modelo == "fSFG" & tipo_ponto == "min" ~ -0.7,
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[2]), ", ", round(10^vetor_prob[3]), ")") & modelo == "fLTG" & tipo_ponto == "min" ~  1.4,
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[2]), ", ", round(10^vetor_prob[3]), ")") & modelo == "fSFG" & tipo_ponto == "max" ~ -0.5,
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[2]), ", ", round(10^vetor_prob[3]), ")") & modelo == "fLTG" & tipo_ponto == "max" ~  1.5,
      
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[3]), ", ", round(10^vetor_prob[4]), ")") & modelo == "fSFG" & tipo_ponto == "min" ~  1.5,
      painel_logvelDisp_e == paste0("[", round(10^vetor_prob[3]), ", ", round(10^vetor_prob[4]), ")") & modelo == "fLTG" & tipo_ponto == "min" ~ -1.0,
...
limites_x <- modelos %>%
  ...
  mutate(
    x_min_exp = x_min - 0.50,
    x_max_exp = x_max + 0.40
  )

padding_df <- limites_x %>%
  mutate(
    y = NA,
    modelo = "fSFG"  # só pra manter coerência com aes()
  ) %>%
